  ...
  while (!O.empty())
  {
    ...
    else if (nodes2D[iPred].isOpen())
    {
      ...
      else
      {
        ...
        //remember delete
        std::vector<Node2D> *obstacleConfigurationPtr = new vector<Node2D>;
        std::vector<Node2D>  *reachableGrid =  new vector<Node2D>;
        configurationSpace.getObsInCircle(nPred, radius, obstacleConfigurationPtr);
        for (int j = 0; j < Node2D::dir_y; j++)
        {
          for (int i = 0; i < Node2D::dir_x; i++)
          {
            ...
            // ensure successor is not blocked by obstacle
            // ensure successor is not on closed list
            if (nSucc->isOnGrid(width, height) && nSucc->isReachable(nPred, &goal)  &&  configurationSpace.isTraversable(nSucc) && !nodes2D[iSucc].isClosed())//without collision test
            {
              if(!nSucc->willCollision(nPred,obstacleConfigurationPtr, &goal))
              // calculate new G value
              nSucc->updateG();
              //newG = nSucc->getG();
              reachableGrid->push_back(*nSucc);
            }
            else
            {
              delete nSucc;
        ...
        nPred->calculateAndSetG(*reachableGrid);
        for( auto iter = reachableGrid->begin(); iter != reachableGrid->end() ; iter++)
        {
          newG = iter->getG();
          iSucc = iter->setIdx(width);
          // if successor not on open list or g value lower than before put it on open list
          if (!nodes2D[iSucc].isOpen() || newG < nodes2D[iSucc].getG())
          {
            // calculate the H value
        ...
        delete obstacleConfigurationPtr;
        delete reachableGrid;
      }
    }
  }
