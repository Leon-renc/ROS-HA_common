...
//R = 13, 3.50 DEG
// const float Node3D::dy[] = {0, -0.0484801, 0.0484801};
// const float Node3D::dx[] = {0.7941248, 0.792643, 0.792643};
// const float Node3D::dt[] = {0, 0.0610865, -0.0610865};

//

const float Node3D::dy[] = {0,
                            -Node3D::r *(1 - cos(Node3D::deg)),
                            Node3D::r *(1 - cos(Node3D::deg))};
const float Node3D::dx[] = {Node3D::r * Node3D::deg ,
                            Node3D::r *sin(Node3D::deg),
                            Node3D::r *sin(Node3D::deg)};
const float Node3D::dt[] = {0,
                            Node3D::deg,
                            -Node3D::deg};
...
bool Node3D::isOnGrid(const int width, const int height) const
{
  ...
  //std::cout << x - map_origin_x << " " << y - map_origin_y << std::endl;
  ...
  return flag_1 && flag_2 && (int)(t / Constants::deltaHeadingRad) >= 0 && (int)(t / Constants::deltaHeadingRad) < Constants::headings;
}
...
bool Node3D::isInRange(const Node3D &goal) const
{
  ...
  return std::sqrt((dx * dx) + (dy * dy)) < Constants::reedssheppShotDistance;
}
...
Node3D *Node3D::createSuccessor(const int i, const float dist)
{
  float xSucc;
  float ySucc;
  float tSucc;
