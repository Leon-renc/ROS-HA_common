...
void DynamicVoronoi::initializeEmpty(int _sizeX, int _sizeY, bool initGridMap)
{
    ...
    // std::cout<<"DATA:"<<bool(data==NULL)<<std::endl;
    ...
    if (data != NULL)
    {
        for (int x = 0; x < sizeX; x++)
        {
            ...
            // std::cout<<"DATA 0:"<<bool(data[0]==NULL)<<std::endl;
            delete[] data[x];
        }
...
void DynamicVoronoi::initializeMap(int _sizeX, int _sizeY, bool **_gridMap)
{
    // open.resetCount();
    gridMap = _gridMap;
    //std::cout << "dynamicvoronoi.cpp line 145: " << open.empty() << std::endl;
    std::cout << "dynamicvorono sizeX:" << _sizeX << "sizeY:" << _sizeY <<std::endl;
...
void DynamicVoronoi::update(bool updateRealDist)
{
    ...
    std::cout << "dynamicvoronoi.cpp line 213: " << open.empty() << std::endl;
    while (!open.empty())
    {
        // std::cout << "dynamicvoronoi.cpp line 216!" << std::endl;
        INTPOINT p = open.pop();
        int x = p.x;
        int y = p.y;
        dataCell c = data[x][y];
