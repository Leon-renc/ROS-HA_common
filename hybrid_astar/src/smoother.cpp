...
void Smoother::smoothPath(DynamicVoronoi& voronoi) {
  ...
  while (iterations < maxIterations) {
    ...
    for (int i = 2; i < pathLength - 2; ++i) {
     ...
     // std::cout<<  "x:" <<  xi.getX() << " y:" << xi.getY() << std::endl;
      ...
      //有时会报错abs values not larger than 0
      // ensure that it is on the grid
...
void Smoother::tracePath(const Node3D* node, int i, std::vector<Node3D> path) {
  ...
  //std::cout << "node->getX()" << node->getX() << "node->getY()" << node->getY() << std::endl;
  i++;
  path.push_back(*node);
...
Vector2D Smoother::obstacleTerm(Vector2D xi) {
  ...
  //for  transform between globalmap and voronoi data quarey  in conch
  float voronoiXoffset =  -133.42;
  float  voronoiYoffset =105.99;
  float obsDst = voronoi.getDistance(xi.getX()+voronoiXoffset, xi.getY()+voronoiYoffset);
   //std::cout<<" distance to the cloest obstacle:"<<obsDst<<std::endl;
