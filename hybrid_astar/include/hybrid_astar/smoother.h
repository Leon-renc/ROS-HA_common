...
public:
  ...
  */
   ...
   {
      //--Mapping debug--
      // if (vec.getX() - map >= 0 && vec.getX() < width &&
      //     vec.getY() >= 0 && vec.getY() < height)
      // {
      //    return true;
      // }
      // return false;
      if ((vec.getX() - map_origin_x) / map_resolution >= 0 && (vec.getX() - map_origin_x) / map_resolution < width &&
          (vec.getY() - map_origin_y) / map_resolution >= 0 && (vec.getY() - map_origin_y) / map_resolution < height)
      {
         return true;
      }
      return false;
