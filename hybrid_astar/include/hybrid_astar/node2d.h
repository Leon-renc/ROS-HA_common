...
class Node2D {
 public:
  ...
  void updateG() { g += movemenTweight(*pred); d = true; }
  /// Updates the cost-to-go for the node x' to the goal node.
  void updateH(const Node2D& goal) { h = movemenTweight(goal); }
  
  /// The heuristic as well as the cost measure.
  float movemenTweight(const Node2D& pred) const { return sqrt((x - pred.x) * (x - pred.x) + (y - pred.y) * (y - pred.y)); }
