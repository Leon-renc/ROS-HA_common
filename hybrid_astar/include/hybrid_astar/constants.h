...
*/
  //--Mapping debug-- 
  // 必须是extern 且不能在这里初始化，要在任意一个cpp里初始化，使用static会为每一个cpp创建同名变量
  extern int map_origin_x;
  extern int map_origin_y;
  extern float map_resolution;
  extern float vehicle_length;
  extern float vehicle_width;
...
/// A flag to toggle the connection of the path via ReedsShepp's shot (true = on; false = off)
...
static const float bloating = 0;
/// [m] --- The width of the vehicle
// 通过影响bbsize来影响collisionlookup
static const float width = 5.15 + 2 * bloating;
/// [m] --- The length of the vehicle
static const float length = 10.8 + 2 * bloating;  
/// [m] --- The minimum turning radius of the vehicle 
// 决定曲线射击终点时的最小半径
// 13  -- 25  ，比真实值大一些，保证能转过来
static const float r = 30; // 25
...
static const float penaltyReversing = 10.0;//2.0;
/// [#] --- A movement cost penalty for change of direction (changing from primitives < 3 to primitives > 2)
static const float penaltyCOD = 4.0;//2.0;
...
static const float reedssheppShotDistance = 40; // 1000;
/// [m] --- The step size for the analytical solution (ReedsShepp shot) primarily relevant for collision checking
