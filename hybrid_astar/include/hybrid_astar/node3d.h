...
class Node3D {
 public:
  ...
  /// set the motion primitive index
  ...
  //--Mapping debug--
  //void setIdx(int width, int height) { this->idx = (int)(t / Constants::deltaHeadingRad) * width * height + (int)(y) * width + (int)(x); return idx;}
  int setIdx(int width, int height) { this->idx = (int)(t / Constants::deltaHeadingRad) * width * height + ((int)((y-map_origin_y)/map_resolution)) * width + (int)((x-map_origin_x)/map_resolution); return idx;}
 ...
 ///reset the closed and open status
  ...
  Node3D*createSuccessor(const int i, const float dist);
